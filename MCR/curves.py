...
class CurveModule(Module):
    ...
    def clone(self, j):
        for i, parameter_name in enumerate(self.parameter_names):
            curr_parameter = getattr(self, '%s' % (parameter_name))
            parameter = getattr(self, '%s_%d' % (parameter_name, j))
            parameter.data.copy_(curr_parameter)
            print( '%s_%d copied' % (parameter_name, j))
...
class Linear(CurveModule):
    ...
    def __init__(self, in_features, out_features, fix_points, bias=True):
        ...
        """self.register_parameter(
            'weight' ,
            Parameter(torch.Tensor(out_features, in_features), requires_grad=False)
        )
        self.register_parameter(
            'bias' ,
            Parameter(torch.Tensor(out_features, in_features), requires_grad=False)
...
class Conv2d(CurveModule):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, fix_points, stride=1,
        ...
        """self.register_parameter(
            'weight' ,
            Parameter(torch.Tensor(out_channels, in_channels // groups, *kernel_size), requires_grad=False)
        )
        self.register_parameter(
            'bias' ,
            Parameter(torch.Tensor(out_channels), requires_grad=False)
        )"""
...
class _BatchNorm(CurveModule):
    ...
    def __init__(self, num_features, fix_points, eps=1e-5, momentum=0.1, affine=True,
        ...
        """self.register_parameter(
            'weight' ,
            Parameter(torch.Tensor(num_features), requires_grad=False)
        )
        self.register_parameter(
            'bias' ,
            Parameter(torch.Tensor(num_features), requires_grad=False)
    ...
    def forward(self, input, t=None):
        if t is None:
            if self.t is None:
                ...
                t = self.t
        coeffs_t = self.coeff_layer(t)
        output = self.net.forward_coeff(input, coeffs_t = coeffs_t)
        self._compute_l2()
        return output
    def fix_t(self, t):
        self.t = t
    def forward_coeff(self, input, coeffs_t):
        return self.net.forward_coeff(input, coeffs_t = coeffs_t)
